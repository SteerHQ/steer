...
4. Попробуйте **Stereo Mix** (встроен в Windows, не требует установки)
5. Если нет Stereo Mix - установите **VoiceMeeter** (бесплатный и стабильный)
...
### 3. Скрытый режим (защита от демонстрации экрана)

🛡️ **Окно ассистента не попадает в демонстрацию экрана и запись** (Zoom, Google Meet, Microsoft Teams, OBS) — для вас оно видно, для собеседующего нет.

- Включён **по умолчанию** при запуске.
- Индикатор в шапке окна: 🛡️ — скрыто от захвата, 👁️ — видно при демонстрации.
- Реализовано через Windows `WDA_EXCLUDEFROMCAPTURE` (требует Windows 10 версии 2004+).

**Глобальные горячие клавиши** (работают, даже когда окно не в фокусе — не нужно переключаться на ассистента во время собеседования):

| Клавиши        | Действие                         |
| -------------- | -------------------------------- |
| `Ctrl+Shift+S` | Включить/выключить скрытый режим |
